...
import { Truck, X, Search, Check, Trash2 } from 'lucide-react';
                          ...
                          <div className="flex items-center justify-end gap-2">
                            {item.status === 'Draft' && (
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  if (window.confirm('Are you sure you want to delete this draft delivery note?')) {
                                    onDelete?.(item);
                                  }
                                }}
...
const DeliveryNotesPage = () => {
  const { data, isLoading, error, mutate } = useFrappeGetCall('customer_portal.api.v1.get_delivery_note_data');
  const { deleteDoc } = useFrappeDeleteDoc();

  const [selectedDeliveryNote, setSelectedDeliveryNote] = useState<any>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);

  const handleViewDeliveryNote = (dn: any) => {
  ...
  const handleDownloadDeliveryNote = (dn: any) => {
    ...
    link.href = pdfUrl;
    link.download = `Delivery Note_${docName}.pdf`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };

  const handleDeleteDeliveryNote = async (dn: any) => {
    try {
      const docName = dn.name || dn.id;
      await deleteDoc('Delivery Note', docName);
      mutate();
    } catch (error) {
      console.error("Failed to delete delivery note:", error);
      alert("Failed to delete the delivery note. Please try again.");
    }
  };

  const columns: Column[] = [
    { label: 'Delivery ID', key: 'name' },
    { label: 'Date', key: 'posting_date' },
    { label: 'Order Ref', key: 'customer_name' },
    { label: 'Status', key: 'status' },
  ];

  if (isLoading) return <div className="p-10 flex items-center justify-center min-h-screen bg-slate-50"><div className="text-slate-500 font-medium animate-pulse">Loading delivery notes...</div></div>;
  if (error) return <div className="p-10 flex items-center justify-center min-h-screen bg-slate-50"><div className="text-rose-500 font-bold">Error loading delivery notes: {error.message}</div></div>;

  const deliveryNotes = data?.message || [];
  ...
  return (
    <div className="p-10 bg-slate-50 min-h-screen">
      <DocumentListView 
        ...
        rawData={deliveryNotes}
      />

      <ViewRecords isOpen={isModalOpen} onClose={handleCloseModal} title="Delivery Note Details">
        {selectedDeliveryNote ? (
          <div className="space-y-2 text-sm">
            <p><strong>Delivery ID:</strong> {selectedDeliveryNote.name}</p>
            <p><strong>Date:</strong> {selectedDeliveryNote.posting_date}</p>
            <p><strong>Order Ref:</strong> {selectedDeliveryNote.customer_name}</p>
