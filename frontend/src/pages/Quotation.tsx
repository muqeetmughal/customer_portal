                          ...
                          <div className="flex items-center justify-end gap-2">
                            {item.status === 'Draft' && (
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  if (window.confirm('Are you sure you want to delete this draft quotation?')) {
                                    onDelete?.(item);
                                  }
                                }}
...
const QuotationsPage = () => {
  const { data, isLoading, error, mutate } = useFrappeGetCall('customer_portal.api.v1.get_quotation_data');
  ...
  const handleDownloadQuotation = (quotation: any) => {
    if (!quotation) return;
    const docName = quotation.name || quotation.id;
    ...
    link.href = pdfUrl;
    link.download = `Quotation_${docName}.pdf`;
  ...
  const handleDeleteQuotation = async (quotation: any) => {
    try {
      const docName = quotation.name || quotation.id;
      await deleteDoc('Quotation', docName);
      mutate();
    } catch (error) {
  ...
  if (isLoading) return <div className="p-10 flex items-center justify-center min-h-screen bg-slate-50"><div className="text-slate-500 font-medium animate-pulse">Loading quotations...</div></div>;
  if (error) return <div className="p-10 flex items-center justify-center min-h-screen bg-slate-50"><div className="text-rose-500 font-bold">Error loading quotations: {error.message}</div></div>;

  const quotations = data?.message || [];
  ...
  return (
    <div className="p-10 bg-slate-50 min-h-screen">
      ...
      <ViewRecords isOpen={isModalOpen} onClose={handleCloseModal} title="Quotation Details">
        {selectedQuotation ? (
          <div className="space-y-2 text-sm">
            <p><strong>Quotation ID:</strong> {selectedQuotation.name}</p>
            <p><strong>Date:</strong> {selectedQuotation.transaction_date}</p>
            <p><strong>Valid Till:</strong> {selectedQuotation.valid_till}</p>
            <p><strong>Total Amount:</strong> {selectedQuotation.grand_total}</p>
